...
Here I am doing xy and you can do yx.
[source](www.url.com)
""")
...
with st.expander("Raw data I used"):
  st.dataframe(apple_stock)
  st.write("TEST")#give overview of data to people

st.header("Method")
st.markdown("""
We want to predict the price of the next 7 days based on data from the last 30 days
""")
...
st.markdown("""
tthis is the code I use
:""")
code = """X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, shuffle=False)"""
st.code(code, language = "python")

tab1, tab2 = st.tabs(["LinReg", "XGBoost"])#tabs uses as input a list of strings, in docs on streamlit.io
...
### Model 1: Basic Direct LinReg
  ...
  st.plotly_chart(create_visualization(y_train, y_fit), use_containter_width = True)
  st.plotly_chart(create_visualization(y_train, y_pred), use_containter_width = True)
